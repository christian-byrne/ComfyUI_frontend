...
describe('BC.06 — Custom canvas drawing (per-node and canvas-level) [v1 → v2 migration]', () => {
  // TODO(I-TF): hydrate via loadEvidenceSnippet('S2.N9', 0) and run against runV1 + runV2 (assert behavioral parity).
  it.todo('S2.N9 — migrates from v1 to v2')

  // TODO(I-TF): hydrate via loadEvidenceSnippet('S3.C1', 0) and run against runV1 + runV2 (assert behavioral parity).
  it.todo('S3.C1 — migrates from v1 to v2')
})
